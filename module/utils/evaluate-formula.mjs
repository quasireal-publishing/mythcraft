import MythCraftRoll from "../rolls/base-roll.mjs";
 ...
 * @param {boolean} [options.strict=false]      Forwarded to {@linkcode MythCraftRoll.evaluateSync}.
 * @param {boolean} [options.allowStrings=true] Forwarded to {@linkcode MythCraftRoll.evaluateSync}.
...
export default function evaluateFormula(formula, rollData = {}, { strict = false, allowStrings = true, contextName = "unknown" } = {}) {
  let result = 0;
  try {
    const evaluatedResult = new MythCraftRoll(formula, rollData).evaluateSync({ strict, allowStrings }).total;
