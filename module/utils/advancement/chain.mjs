...
export default class AdvancementChain {
  ...
  constructor(actor, levelRange) {
    ...
    if (actor.type !== "character") {
      throw new Error("Unable to create AdvancementChain for actor types other than 'character'.");
    }
   ...
   * @returns {MythCraftItem | null} Returns null if the actor doesn't have a class already and this isn't to give one,
  ...
  * activeNodes() {
   ...
   * @param {MythCraftItem} [options.item]          A single compendium item being added to an actor that needs to apply advancements.
   ...
   * @param {MythCraftItem} item                    An item that has an Advancement collection.
   * @param {object} [options]
